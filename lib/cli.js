...
function CLI(defaults){
  ...
  this._build = function(cb){
    ...
    if (defaults.withArgs.indexOf('story') > -1){
      data.push(buildStory(opts))
    }
  ...
  function buildComponent(opts){
    ...
    return {
      type: opts.ext === 'jsx' ? constants.TYPE_COMPONENT_JSX : constants.TYPE_COMPONENT_JS ,
      data,
      file: 'index' }
  }
